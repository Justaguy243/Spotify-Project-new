...
async function getSongs(folder) {
  currFolder = folder;
  let a = await fetch(`https://spotify79.netlify.app/${folder}/`);
  ...
  for (const song of songs) {
    ...
    let b = await fetch(`https://spotify79.netlify.app/Author.json`)
...
const Getimage = (image) => {
  console.log(image);
  let haveimage = "https://spotify79.netlify.app/spotify_images/" + image.replaceAll("%20", " ").replaceAll(".mp3", ".jpg");
  return haveimage;
}
...
async function DisplayAlbums() {
  let b = await fetch(`https://spotify79.netlify.app/songs/`);
  ...
  for (let index = 0; index < array.length; index++) {
    ...
    if (e.href.includes("/songs") && !e.href.includes(".htaccess")) {
      ...
      let b = await fetch(`https://spotify79.netlify.app/${folder}/info.json`)
      let response2 = await b.json()
